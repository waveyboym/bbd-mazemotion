...
io.on("connection", (socket) => {
  ...
  socket.on("createRoom", () => {
    ...
    while (ExistingRoom) {
      ...
      if (!rooms[roomId]) {
        rooms[roomId] = { maze: [], Users: [], goal: null };
        socket.join(roomId);
  ...
  socket.on("startGame", (roomId) => {
    ...
    //Assigning goalpoint
    const goal = getRandomSpotforgoal(rooms[roomId].maze);
    ...
    if (viewers[roomId]) {
      viewers[roomId].forEach((viewer) => {
        socket.to(viewer).emit("gameStarted", {
          maze: rooms[roomId].maze,
          Users: rooms[roomId].Users,
          goal: rooms[roomId].goal,
        });
  ...
  socket.on("updatePosition", (data) => {
    ...
    socket
      .to(data.roomId)
      .emit("updatePosition", { Users: rooms[roomId].Users });
...
function getRandomSpotforgoal(maze) {
  let x = Math.floor(Math.random() * maze[0].length);
  let y = Math.floor(Math.random() * maze.length);
  while (maze[y][x] !== 0) {
    x = Math.floor(Math.random() * maze[0].length);
